...
begin
  ...
  PROC: process (clk, reset, enable)
    ...
    variable sequence_len_store: unsigned(3 downto 0) := "0000";
    variable next_len: unsigned(4 downto 0) := "00000";
    variable finished_store: boolean := false;
  begin
    if (reset = '1') then
      sequence_store := (others => BLUE);
      sequence_len_store := "0000";
      ...
      next_len := resize(sequence_len_store, 5) + 1; -- might be better to use a
                                                     -- shift register for this
      if (next_len > 8) then
        -- report "got to the overflow guard";
        finished_store := true;
      else
        sequence_len_store := next_len(3 downto 0);
      end if;
    end if;
